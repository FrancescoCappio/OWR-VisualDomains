...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, num_classes_est=50, relu=True, sagnet=False):
        ...
        self.linear = nn.Linear(64, num_classes_est) #  NCM_classifier(64, num_classes, deep_nno)
        ...
        if self.sagnet: 
            ...
            self.style_linear = nn.Linear(64, num_classes_est) # NCM_classifier(64, num_classes, deep_nno)

        self.num_classes = num_classes # current effective number of classes
        self.num_classes_est = num_classes_est # max number of classes
    ...
    def predict(self, x, x_style=None):
        ...
        if self.sagnet and x_style is not None:
            ...
            return (out, out, 1/out), (out_style, out_style, 1/out_style)
    ...
    def add_classes(self, new_classes):
        ...
        assert self.num_classes <= self.num_classes_est, "Reached the maximum number of classes!"
...
def ResNet18(classes, pretrained, relu=True, sagnet=False):
    net = ResNet(BasicBlock, [3, 3, 3], num_classes=classes, relu=relu, sagnet=sagnet)
